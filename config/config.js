...
const replacePassword = (uri, pwd) =>
  uri.replace("<password>", encodeURIComponent(pwd || ""));
...
const config = {
  ...
  jwt: {
    ...
    issuer:
      process.env.JWT_ISSUER || process.env.APP_NAME || "Node Template API",
    audience:
      process.env.JWT_AUDIENCE || process.env.APP_NAME || "Node Template API",
  },
  ...
  security: {
    ...
    cors: {
      ...
      methods: splitCSV(process.env.CORS_METHODS, [
        "GET",
        "POST",
        "PUT",
        "PATCH",
  ...
  logging: {
    morganFormat:
      process.env.MORGAN_FORMAT || (isProduction ? "combined" : "dev"),
  },
  errorHandling: {
